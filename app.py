...
GOOGLE_CLIENT_ID = '198271245790-rne4mde5ivm3hrqkfoqu73f3cjci6voj.apps.googleusercontent.com'
client_secrets_file = os.path.join(pathlib.Path(__file__).parent,
                                   "client_secret.json")  # Enter your updated client_secret.json data
...
app.config['MONGO_URI'] = 'mongodb+srv://pidan321:pidan321@cluster0.yq8rugc.mongodb.net/test'
mongo.init_app(app)
...
def callback():
    ...
    if result == None:
        session['user_id'] = str(IRS_USERS.insert_one(
            {"Name": id_info.get("name"), "Email": id_info.get("email"), "Google_id": id_info.get("sub")}).inserted_id)
    ...
    if 'user_id' in session and 'user_name' in session:
        try:
            ...
            if file and allowedExtension(file.filename):
                temp = resumeFetchedData.find_one({"UserId": ObjectId(session['user_id'])}, {"ResumeTitle": 1}) # yo line ma error aako ho hai sandesh maila debug garda tha pako
                if temp == None:
                    pass
                else:
                ...
                fetchedData = extractorObj.extractorData("static/resumes/" + filename,
                ...
                result = resumeFetchedData.insert_one(
                    {"UserId": ObjectId(session['user_id']), "Name": fetchedData[0], "Mobile_no": fetchedData[1],
                     "Email": fetchedData[2], "Skills": list(fetchedData[3]), "Education": fetchedData[4], "Appear": 0,
                     "ResumeTitle": filename, "ResumeData": fetchedData[5]})
                if result == None:
                    return render_template("EmployeeDashboard.html", errorMsg="Problem in Resume Data Storage")
                else:
                    result1 = Ranked_resume.insert_one(
                        {"UserId": ObjectId(session['user_id']), "Top_skills": dict(skillsPercentage)})
                    if result1 == None:
...
def empSearch():
    if request.method == 'POST':
        ...
        TopEmployeers = None
        TopEmployeers = Ranked_resume.find({"Top_skills." + category: {"$ne": None}},
                                           {"Top_skills." + category: 1, "UserId": 1}).sort(
            [("Top_skills." + category, -1)])
