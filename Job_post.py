...
def JOB_POST():
    fetched_jobs = None
    fetched_jobs = JOBS.find({}, {"_id": 1, "Job_Profile": 1, "CompanyName": 1, "CreatedAt": 1,
                                  "Job_description_file_name": 1, "LastDate": 1, "Salary": 1}).sort([("CreatedAt", -1)])
    if fetched_jobs == None:
        ...
        for i in fetched_jobs:
            jobs[cnt] = {"job_id": i['_id'], "Job_Profile": i['Job_Profile'], "CompanyName": i['CompanyName'],
                         "CreatedAt": i['CreatedAt'], "Job_description_file_name": i['Job_description_file_name'],
                         'LastDate': i['LastDate'], "Salary": i['Salary']}
            cnt += 1
...
def ADD_JOB():
    try:
        ...
        result = JOBS.insert_one(
            {"_id": jd_id, "Job_Profile": job_profile, "Job_Description": fetchedData, "CompanyName": company,
             "LastDate": last_date, "CreatedAt": datetime.now(), "Job_description_file_name": filename,
             "Salary": salary})
        if result == None:
...
def APPLY_JOB():
    ...
    emp_data = resumeFetchedData.find_one({"UserId": ObjectId(session['user_id'])}, {"Skills"})
    match_percentage = job_compare_obj.match(str(jd_data['Job_Description']), str(emp_data['Skills']))
    result = None
    result = Applied_EMP.insert_one(
        {"job_id": ObjectId(job_id), "user_id": ObjectId(session['user_id']), "User_name": session['user_name'],
...
def view_applied_candidates():
    job_id = request.form['job_id']
    result_data = None
    result_data = Applied_EMP.find({"job_id": ObjectId(job_id)}, {"User_name": 1, "Matching_percentage": 1}).sort(
